...
- [x] **RUN-05**: `image_op` and `image_task` return `runId` in response so users can locate intermediates
...
- [x] **PRIM-08**: `analyze_ocr` returns extracted text via `tesseract.js`
...
- [x] **TASK-04**: `dry_run: true` returns the Plan without executing; user can preview estimated cost and provider choices
